...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        goal_state = self.startingPosition
        if not(state == goal_state):
            visited_corner = state[1]
            if (visited_corner == (True, True, True, True)):
                return state
...
def cornersHeuristic(state, problem):
    ...
    if not (state == problem.startingPosition):
        postition_state = state[0]
        if (state[1][0] == False):
            unvisited_corners.append(problem.corners[0])
        if (state[1][1] == False):
            unvisited_corners.append(problem.corners[1])
        if (state[1][2] == False):
            unvisited_corners.append(problem.corners[2])
        if (state[1][3] == False):
            unvisited_corners.append(problem.corners[3])
    else:
        postition_state = state
        unvisited_corners = list(problem.corners)

    unvisited_corners_number = len(unvisited_corners)

    if unvisited_corners_number == 0:
        total_distance = 0

    while unvisited_corners_number > 0:
        distance_to_all_corners = []
        for selected_corner in unvisited_corners:
            xy1 = postition_state
            xy2 = selected_corner
            distance_to_corner = abs(xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])
            distance_to_all_corners.insert(-1,(distance_to_corner, selected_corner))

        closest_corner = min(distance_to_all_corners)
        unvisited_corners.remove(closest_corner[1])
...
def foodHeuristic(state, problem):
    ...
    available_food = []
    total_distance = 0
    for xi in range(0, foodGrid.width):
        for xj in range(0, foodGrid.height):
            if (foodGrid[xi][xj] == True):
                food_location = (xi, xj)
                available_food.append(food_location)

    available_food_number = len(available_food)

    if available_food_number == 0:
        return 0

    max_distance = ((0, 0), (0, 0), 0)

    for current_food in available_food:
        for select_food in available_food:
    ...
    if (max_distance[0] == (0, 0) and max_distance[1] == (0, 0)):
        hvalue = manhattanDistance(position, available_food[0])
    else:
